...
import { TOOL_NAMES, toolInputSchemas as toolInputSchemas2, toolOutputSchemas as toolOutputSchemas2 } from "@foundry-mcp/shared";
...
var ApprovalStore = class {
  ...
  create(toolName, payload, requestedBy) {
    const tokenId = `approval_${randomBytes(24).toString("hex")}`;
    const expiresAt = new Date(Date.now() + this.ttlSeconds * 1e3).toISOString();
    const record = {
      expiresAt,
      payload,
  ...
  consume(tokenId, toolName, payload) {
    ...
    if (record.expiresAt < (/* @__PURE__ */ new Date()).toISOString()) {
...
function loadConfig(env = process.env) {
  return {
    allowedOrigins: (env.BRIDGE_ALLOWED_ORIGINS ?? "http://127.0.0.1").split(",").map((value) => value.trim()).filter(Boolean),
...
import { BridgeError as BridgeError2, ERROR_CODES as ERROR_CODES2 } from "@foundry-mcp/shared";
var JobQueue = class {
  ...
  async enqueueAndWait(job, timeoutMs) {
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        ...
        reject(new BridgeError2(ERROR_CODES2.timeout, "Timed out waiting for the Foundry bridge module.", 504));
      }, timeoutMs);
  ...
  async claimNext(sessionId, waitMs) {
    const deadline = Date.now() + waitMs;
    while (Date.now() < deadline) {
      const next = [...this.pending.values()].filter((entry) => !entry.claimedBy).sort((left, right) => left.createdAt - right.createdAt)[0];
      if (next) {
  ...
  complete(jobId, result) {
    ...
    if (!pending) {
      throw new BridgeError2(ERROR_CODES2.invalidPayload, "Job result did not match a pending job.", 400);
    }
    ...
    if (result.status === "error") {
      pending.reject(
        new BridgeError2(
          result.error?.code ?? ERROR_CODES2.moduleError,
...
var SessionRegistry = class {
  ...
  getActive() {
    const active = [...this.sessions.values()].filter((session) => session.lastSeenAt + this.sessionTtlMs >= Date.now()).sort((left, right) => right.lastSeenAt - left.lastSeenAt)[0];
    return active;
  }
};

// src/services/tool-runner.ts
import {
  BridgeError as BridgeError3,
...
var ToolRunner = class {
  ...
  async runTool(toolName, rawInput) {
    const schema = toolInputSchemas[toolName];
    const parsedInput = schema.parse(rawInput);
    const requestContext = parsedInput.requestContext;
    ...
    try {
      ...
      const bridgeError = error instanceof BridgeError3 ? error : new BridgeError3(ERROR_CODES3.moduleError, error instanceof Error ? error.message : "Unknown error", 500);
      const envelope = {
        audit: {
  ...
  async executeModuleTool(toolName, input, requestId, requestContext) {
  ...
  async executeWriteTool(toolName, input, requestId, approvalRecord) {
    this.assertBridgeOnline();
    ...
    if (!dryRun && !approvalRecord) {
      ...
      if (typeof approvalToken !== "string") {
        throw new BridgeError3(
          ERROR_CODES3.approvalRequired,
  ...
  async previewChange(input, requestId) {
    const change = input.change;
    if (change.payload.dryRun !== true) {
      throw new BridgeError3(ERROR_CODES3.invalidPayload, "preview_change payloads must set dryRun=true.", 400);
    }
    ...
    delete payloadForApproval.approvalToken;
  ...
  async applyApprovedChange(input, requestId) {
    const approval = this.deps.approvalStore.consume(input.approvalToken);
  ...
  assertBridgeOnline() {
    if (!this.deps.sessionRegistry.getActive()) {
      throw new BridgeError3(
        ERROR_CODES3.bridgeOffline,
        "No active Foundry bridge session is connected. Keep the dedicated GM session open.",
        503
      );
    }
  }
  extractReadData(toolName, after) {
    if (!after) {
      throw new BridgeError3(ERROR_CODES3.moduleError, "Module did not return data.", 500);
    }
    switch (toolName) {
      case "find_documents":
        return { documents: after.documents ?? [] };
      case "get_document":
        return { document: after.document };
      case "list_folders":
        return { folders: after.folders ?? [] };
...
// src/http/auth.ts
function createModuleAuth(config2) {
  return async function moduleAuth(request, reply) {
    const header = request.headers.authorization;
    if (header !== `Bearer ${config2.sharedToken}`) {
...
var registerSchema = external_exports.object({
  foundryUserId: external_exports.string().min(1),
  foundryUserName: external_exports.string().min(1).optional(),
  moduleVersion: external_exports.string().min(1),
  worldId: external_exports.string().min(1).optional(),
  worldTitle: external_exports.string().min(1).optional()
}).strict();
...
var claimSchema = external_exports.object({
  sessionId: external_exports.string().min(1),
  waitMs: external_exports.number().int().min(0).max(3e4).default(1e4)
}).strict();
var resultSchema = external_exports.object({
  result: external_exports.object({
    after: external_exports.record(external_exports.unknown()).nullable(),
    before: external_exports.record(external_exports.unknown()).nullable(),
    diff: external_exports.array(
      external_exports.object({
        after: external_exports.unknown(),
...
function buildApp(deps) {
  ...
  app2.get("/version", async () => ({
    name: "foundry-mcp-bridge-server",
    version: "0.1.0"
  }));
  app2.register(async (moduleRoutes) => {
    ...
    moduleRoutes.post("/jobs/:jobId/result", async (request, reply) => {
      ...
      deps.jobQueue.complete(request.params.jobId, parsed.result);
      return reply.send({ ok: true });
    });
  }, { prefix: "/module" });
  return app2;
...
var approvalStore = new ApprovalStore(config.approvalTtlSeconds);
...
var registeredToolNames = TOOL_NAMES;
for (const toolName of registeredToolNames) {
  server.registerTool(
    toolName,
    {
      description: TOOL_DEFINITIONS[toolName].description,
      inputSchema: toolInputSchemas2[toolName].shape,
      outputSchema: toolOutputSchemas2[toolName].shape,
